...
parse.add_argument('--data_path', default='./data/', type=str,
                   help='数据集文件夹')
parse.add_argument('--output_dir', default='./bert/output',
                   type=str, help='模型保存文件夹')
parse.add_argument('--best_step', default=600, type=int, help='最佳迭代次数')
...
parse.add_argument('--num_train_epoch', default=30, type=int, help='Epoch')
parse.add_argument('--logging_step', default=50, type=int, help='打印间隔')
parse.add_argument('--eval_step', default=100, type=int, help='验证间隔')
parse.add_argument('--save_step', default=100, type=int, help='保存间隔')
...
def predict(model, dataset, args, eval=False):
    ...
    for _, batch in enumerate(eval_iterator):
        ...
        with torch.no_grad():
            ...
            scores = torch.cat(
                [scores, torch.max(torch.softmax(logits, dim=1), axis=1).values.cpu()])
            if eval:
                labels = torch.cat([labels, batch[3].cpu().squeeze(1)])

    return (pred, labels) if eval else (pred, scores)
...
def main():
    ...
    if args.mode == 'train':
        ...
        model = BertForSequenceClassification.from_pretrained(
            'bert-base-uncased', config=config).to(device)
        # model.num_labels = 3
        ...
        # model.num_labels = 3
        pred, scores = predict(model, dataset, args)
        pred = pd.Series(pred.numpy().tolist())
        scores = pd.Series(scores.numpy().tolist())
        res_csv = pd.concat([dataset.df['news_id'], pred, scores], axis=1)
        res_csv.to_csv(os.path.join(args.output_dir, 'bert_result.csv'),
